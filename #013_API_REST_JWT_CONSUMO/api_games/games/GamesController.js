...
router.post("/game",auth,(req, res) => {
   ...
   Games.create({
       ...
       console.log("Ocorreu um erro na atualização do item: " + error);
        res.sendStatus(400);
   });
...
router.post("/auth",(req, res) => {
    ...
    if(email != undefined){
        ...
        let user =  listUserDB.users.find(u => u.email == email);
        ...
        if(user != undefined){
            ...
            if(user.password == password){
                ...
                res.status(401);
                res.json({error: "Credencial inválida"});
            }
